...
// Helper to revoke an achievement
async function revokeAchievement(userId: string, code: string): Promise<boolean> {
  try {
    const achievement = await db.achievement.findUnique({ where: { code } });
    if (!achievement) return false;
...
// DELETE - Remove a specific earned badge for a user
